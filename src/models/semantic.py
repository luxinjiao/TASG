...
class SemanticSegmentationModule(LightningModule):
    ...
    def test_step(self, batch: NAG, batch_idx: int) -> None:
        ...
        #假设你的 Tensor 是 tensor
        tensor = batch[0].y.cpu()  # 如果 Tensor 在 GPU 上，先转移到 CPU
        np_array = tensor.numpy()  # 转为 Numpy 数组

        # 禁用 Numpy 的打印截断
        np.set_printoptions(threshold=np.inf, linewidth=np.inf)

        # 根据 batch_idx 动态生成文件名
        file_name = f"{batch_idx}_y.h5"

        # 保存数据到 h5 文件
        with h5py.File(file_name, 'w') as f:
            f.create_dataset('tensor_data', data=np_array, compression="gzip")
