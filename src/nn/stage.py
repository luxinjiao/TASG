...
from modules.SFE_utils import SFE
...
class Stage(nn.Module):
    ...
    def forward(
        ...
        if edge_index is not None:
            # superpoint offset
            constructor = SFE(k=3,in_channel=10, out_channel=128).cuda()
            sorted_indices = torch.argsort(super_index)
            sorted_super_index = super_index[sorted_indices]
            sorted_pos = pos[sorted_indices]
            unique_indices, counts = torch.unique(sorted_super_index, return_counts=True)
            offset = torch.cat([torch.tensor([0], device=counts.device), counts.cumsum(dim=0)])
            offset = offset.to(dtype=torch.int32)
            new_feature_sorted = constructor(sorted_pos, offset)  # 注意是 sorted_pos 而不是 pos
            inverse_indices = torch.argsort(sorted_indices)
            surface_feature = new_feature_sorted[inverse_indices]
            x = self.feature_fusion(x, surface_feature)
            #Quantile offset
            # constructor = SFE(k=3, in_channel=13, out_channel=128).cuda()
            # # moffset = len(pos)
            # # n = 1  
            # # offset = torch.linspace(0, moffset, n + 1, dtype=torch.int).cuda()
            # sorted_indices = torch.argsort(super_index)
            # sorted_super_index = super_index[sorted_indices]
            # sorted_pos = pos[sorted_indices]
            # unique_indices, counts = torch.unique(sorted_super_index, return_counts=True)
            # offset = torch.cat([torch.tensor([0], device=counts.device), counts.cumsum(dim=0)])
            # offset = offset.to(dtype=torch.int32)
            # new_feature = constructor(pos, offset)
            # x = self.feature_fusion(x,new_feature)
